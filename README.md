...
| findAll                  |    either returns paginated/all crous restaurants or paginated/all favorites. Data is returned in its reduced form     | page: number, rows: number, offset: number, sortBy: string, fav: number, , geoloc: number, , refresh: number, | current, next, last and first page, number of rows, restaurants (numbers and ReducedCrousDto |
